...
export async function startServer(): Promise<StartedServer> {
  ...
  const heartbeat = config.heartbeatSchedulerEnabled
    ? heartbeatService(db as any, {
        pluginWorkerManager,
        // Fork options. Upstream moved construction here from the block below,
        // so these travel with it rather than staying at the old call site.
        companyMaxConcurrentRuns: config.companyMaxConcurrentRuns,
        mentionedSkillInjectionEnabled: config.mentionedSkillInjectionEnabled,
      })
    : null;
